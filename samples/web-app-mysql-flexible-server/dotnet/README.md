...
| [`secret.yml`](scripts/secret.yml) | Creates the Secret holding sensitive values (the MySQL password and the `SECRET_KEY` the app derives its Data Protection key ring from (so antiforgery tokens and flash messages are valid on every replica)) passed to the app as environment variables. |
...
The app also exposes `GET /health`, the endpoint the liveness and readiness probes call: it returns `{"status": "ok"}` when the MySQL flexible server is reachable and `503` with `{"status": "unavailable"}` otherwise.
...
```bash
kubectl logs deployment/vacation-planner-mysql -n vacation-planner-mysql --tail=50
```

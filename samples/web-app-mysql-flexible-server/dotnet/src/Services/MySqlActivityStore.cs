...
public sealed class MySqlActivityStore(MySqlOptions options, ILogger<MySqlActivityStore> logger) : IActivityStore
{
    ...
    // UseAffectedRows makes UPDATE report changed rows rather than matched rows, as PyMySQL does by default.
    ...
    /// <summary>Reports a change only when a row actually changed, as the Python sample's PyMySQL <c>rowcount</c> check does.</summary>

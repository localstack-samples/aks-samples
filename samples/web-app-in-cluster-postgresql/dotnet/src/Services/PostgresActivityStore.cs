...
public sealed class PostgresActivityStore(PostgresOptions options, ILogger<PostgresActivityStore> logger) : IActivityStore
{
    ...
    // Negotiate TLS when the server offers it, without certificate verification (libpq's "prefer", which the
    // Python sample relies on): the flexible server's certificate is publicly trusted on Azure but self-signed
    // under LocalStack, and the in-cluster StatefulSet offers no TLS at all. Npgsql only validates certificates
    // with SslMode VerifyCA/VerifyFull.
    ...
    /// <summary>Reports a change when a row matched, as the Python sample's psycopg2 <c>rowcount</c> check does.</summary>
    ...
    public async Task<bool> DeleteAsync(string id, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogInformation("Activity '{Id}' did not exist: already deleted.", id);
        }

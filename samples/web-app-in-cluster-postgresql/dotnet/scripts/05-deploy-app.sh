...
SECRET_KEY=$(kubectl get secret vacation-planner-postgres-in-cluster-secrets --namespace $NAMESPACE --output jsonpath='{.data.SECRET_KEY}' 2>/dev/null | base64 --decode 2>/dev/null)
...
# Create secret with the PostgreSQL password and the SECRET_KEY
cat secret.yml |
yq "(.metadata.namespace)|="\""$NAMESPACE"\" |

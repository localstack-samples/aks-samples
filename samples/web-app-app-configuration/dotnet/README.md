...
This sample demonstrates an ASP.NET Core Razor Pages single-page web application called *Vacation Planner* hosted on an [Azure Kubernetes Service (AKS)](https://learn.microsoft.com/en-us/azure/aks/what-is-aks) cluster in the cloud on Azure or locally in the LocalStack emulator for Azure. The app runs in a dedicated namespace and stores activity data in the `activities` table of the `PlannerDB` database on an [Azure Database for PostgreSQL flexible server](https://learn.microsoft.com/en-us/azure/postgresql/flexible-server/overview), exactly like [web-app-postgresql-flexible-server](../../web-app-postgresql-flexible-server/dotnet/README.md).
...
The application itself has **no Azure SDK dependency and no credential of its own** — `VacationPlanner.csproj` references no `Azure.*` package, only `Npgsql`. The provider resolves everything before the pod starts, so the same image would run unchanged against a hand-written ConfigMap. The [App Service version of this sample](https://github.com/localstack/localstack-azure-samples/tree/main/samples/web-app-app-configuration/dotnet) takes the other approach and talks to App Configuration in-process; see [Which approach to use](#which-approach-to-use) for the trade-off.
...
`SECRET_KEY` is what the Python version signs its Flask session cookie with; here it seeds the ASP.NET Core [Data Protection](https://learn.microsoft.com/en-us/aspnet/core/security/data-protection/introduction) key ring, so all replicas validate each other's antiforgery tokens.
...
| | This sample (Kubernetes provider) | [App Service version](https://github.com/localstack/localstack-azure-samples/tree/main/samples/web-app-app-configuration/dotnet) (in-process provider) |
| --- | --- | --- |
| Where configuration is resolved | Outside the process, by the controller | Inside the process, by the Azure SDK |
| Application dependencies | None | `Microsoft.Azure.AppConfiguration.AspNetCore`, `Azure.Identity` |
...
The app logs one line per request — a middleware writes one entry for every call, the probes included, matching the access log the Python image produces — plus one line per database read and write, one line for every activity added, updated or deleted, and one line at startup naming the configuration version it booted with. The [Python version](../python/README.md) writes the same trace.

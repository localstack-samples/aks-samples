...
| [`secret.yml`](scripts/secret.yml) | Creates the Secret holding sensitive values (the SQL password and the `SECRET_KEY` the app derives its Data Protection key ring from (so antiforgery tokens and flash messages are valid on every replica)) passed to the app as environment variables. |

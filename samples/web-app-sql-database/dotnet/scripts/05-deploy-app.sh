...
SECRET_KEY=$(kubectl get secret vacation-planner-sql-secrets --namespace $NAMESPACE --output jsonpath='{.data.SECRET_KEY}' 2>/dev/null | base64 --decode 2>/dev/null)
...
# Create secret with the SQL password and the SECRET_KEY
cat secret.yml |

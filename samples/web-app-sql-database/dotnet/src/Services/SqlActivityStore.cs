...
public sealed class SqlActivityStore(SqlOptions options, ILogger<SqlActivityStore> logger) : IActivityStore
{
    ...
    // Encrypt + TrustServerCertificate, like the Python sample's ODBC connection string: the emulator's SQL Server
    // presents a self-signed certificate, so the connection is encrypted without validating the certificate chain.
    private readonly string _connectionString = new SqlConnectionStringBuilder
    {
        ...
        ConnectTimeout = 30,
        UserID = options.UseAzureCredential ? "" : options.User!,
        Password = options.UseAzureCredential ? "" : options.Password!,
    }.ConnectionString;

    public async Task InitializeAsync(CancellationToken cancellationToken)
    {
        // The table is provisioned by the deployment scripts; just prove the database is reachable.
    ...
    /// <summary>Reports success when the INSERT returned the new row, as the Python sample's <c>OUTPUT INSERTED</c> check does.</summary>
    public async Task<bool> AddAsync(string text, CancellationToken cancellationToken)
    {
        await using var connection = await OpenAsync(cancellationToken);
        await using var command = new SqlCommand(
            """
    ...
    public async Task<bool> UpdateAsync(string id, string text, CancellationToken cancellationToken)
    {
        await using var connection = await OpenAsync(cancellationToken);
        await using var command = new SqlCommand(

...
| [`secret.yml`](scripts/secret.yml) | Creates the Secret holding sensitive values (the `SECRET_KEY` the app derives its Data Protection key ring from (so antiforgery tokens and flash messages are valid on every replica), and the optional connection string / client secret fallback) passed to the app as environment variables. |

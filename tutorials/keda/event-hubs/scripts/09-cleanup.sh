...
# producer Job and the KEDA resources. Pass --disable-keda to also turn the add-on off on the
# cluster (az aks update --disable-keda), which removes the KEDA components and CRDs.
#
# The shared managed identity, its role assignments and the keda-operator annotation are left in
# place on purpose: the other two KEDA tutorials use them. Delete the Azure resources with
# `az group delete --name local-rg` when you are done with all of them.
